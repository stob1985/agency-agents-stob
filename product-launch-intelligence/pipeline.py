...
Runs 5 Claude agents in sequence, each receiving all previous outputs as context.
"""
...
AGENT_SEQUENCE = [
    ("trend_researcher",   "Trend Researcher",        "trend_researcher.md"),
    ("ad_creative",        "Ad Creative Strategist",  "ad_creative.md"),
    ("ugc_writer",         "UGC Writer",              "ugc_writer.md"),
    ("unit_economics",     "Unit Economics Analyst",  "unit_economics.md"),
    ("execution_planner",  "Execution Planner",       "execution_planner.md"),
]
...
def _load_system_prompt(filename: str) -> str:
    """Load an agent system prompt from the agents/ directory."""
...
def _build_user_message_for_agent(
    agent_key: str,
    agent_name: str,
    product: ProductData,
    ...
    """
    Build the user turn for a given agent.
    ...
    if agent_key == "trend_researcher":
        return (
            f"{product_block}\n\n"
            "---\n\n"
            "Please analyze this product and provide your full Trend Research report."
        )

    # Build a short summary of what has already been produced (the LLM already
    # has the full text in the assistant turns above — this just orients it).
...
def _stream_agent(
    ...
    """
    Stream a single agent call. Returns (output_text, input_tokens, output_tokens).
...
def run_pipeline(product: ProductData, verbose: bool = True) -> PipelineResult:
    """
    ...
    for agent_key, agent_name, prompt_file in AGENT_SEQUENCE:
        system_prompt = _load_system_prompt(prompt_file)

        # Build the next user message and append it
        user_msg = _build_user_message_for_agent(
            agent_key, agent_name, product, result.agent_results
        )
        messages.append({"role": "user", "content": user_msg})

        # Stream this agent's response
        output_text, in_tok, out_tok = _stream_agent(
            client=client,
